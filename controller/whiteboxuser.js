...
function getLike(id)
{
	...
	ajax.open("GET", '../controller/data_user.php?likepic='+id, false);
	ajax.setRequestHeader('Content-Type', 'text');
	ajax.send();
	if (ajax.status == 200)
	{
...
{
	var ajax = new XMLHttpRequest();
	ajax.open("GET", '../controller/data_user.php?idnbcom='+id, false);
	ajax.setRequestHeader('Content-Type', 'text');
	ajax.send();
	if (ajax.status == 200)
	{
...
{
	var ajax = new XMLHttpRequest();
	ajax.open("POST",'../controller/data_user.php', false);
